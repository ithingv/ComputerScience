...
- **기수정렬**(radix sort)은 기수 별로 비교 없이 수행하는 정렬 알고리즘이다. 기수로는 정수, 낱말 등 다양한 자료를 사용할 수 있으나 크기가 유한하고 **사전순**으로 정렬할 수 있어야 한다. **버킷정렬**의 일종으로 취급되기도 한다. 기수에 따라 원소를 버킷에 집어 넣기 때문에 비교 연산이 불필요하다. 기수 정렬은 정렬 방법의 특수성 때문에, 부동소스점 실수처럼 특수한 비교 연산이 필요한 데이터에는 적용할 수 없지만 적당한 도메인에서 최적화 된 기수 정렬은 매우 빠를 수 있다.
...
- **복잡도**
    ...
    -  유효숫자가 두개 이상인 경우 모든 숫자 요소에 대해 수행될 때까지 각 자리수에 대해 반복한다. 따라서 전체 시간 복잡도는 O(nw)(w는 기수의 크기)이 된다. 정수와 같은 자료의 정렬 속도가 매우 빠르다.
    -  데이터 전체 크기에 기수 테이블(도메인)의 크기만한 메모리가 더 필요하다.
      
        ```
        최악 시간복잡도: O(nw), n은 키의 수, w는 키의 길이
        공간복잡도: O(n+k), k는 기수의 도메인 크기
        ```
